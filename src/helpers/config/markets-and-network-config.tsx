...
import {
  CustomAvalancheMarket,
  CustomMarket,
  marketsData as _marketsData,
} from '../../ui-config/markets/index';
import {
...
export const marketsData = Object.keys(_marketsData).reduce((acc, value) => {
  acc[value] = _marketsData[value as keyof typeof CustomAvalancheMarket];
  if (
    FORK_ENABLED &&
    _marketsData[value as keyof typeof CustomAvalancheMarket].chainId === FORK_BASE_CHAIN_ID
  ) {
    acc[`fork_${value}`] = {
      ..._marketsData[value as keyof typeof CustomAvalancheMarket],
