...
import { marketsTokensOrder } from '../../../ui-config/assets';
...
export function DynamicPoolDataProvider({ children }: PropsWithChildren<{}>) {
  ...
  const formattedPoolReserves: ComputedReserveData[] = rawReserves
    .map((reserve) => {
    ...
    .sort(
      ({ symbol: a }, { symbol: b }) =>
        marketsTokensOrder.indexOf(a) - marketsTokensOrder.indexOf(b)
    );
