...
import useAddChainId from '../../components/SwitchChainHooks/useAddChainId';
import useSwitchChainId from '../../components/SwitchChainHooks/useSwitchChainId';
...
export function Web3Provider({
  ...
  const intl = useIntl();
  const { library, account, activate, error, deactivate, chainId } =
  ...
  const handleActivation = async (
    ...
    try {
      ...
      isSuccessful = true;
      await useAddChainId(provider);
      await useSwitchChainId(provider);
    } catch (e) {
  ...
  return (
    ...
    >
      ...
      {(!account || !library || !currentAccount || !isAllowedChain(chainId)) && (
        <ConnectWalletModal
